...
void Caffe2ParserBase::ParseInputLayer()
{
 ...
 void Caffe2ParserBase::ParseFCLayer(const caffe2::OperatorDef& op)
 {
    ...
    GetDataFromBlob(b.arg(1),biasData);
     ...
     armnn::IConnectableLayer* fullyConnectedLayer = m_Network->AddFullyConnectedLayer(tensorFullyConnectedDescriptor, weights, Optional<ConstTensor>(biases),op.type().c_str());
     //the output shape = M x shape of bias
     unsigned int outputsize =static_cast<unsigned int>(b.arg(1).floats_size());
...
{   
    ...
    returnLayer = m_Network->AddDepthwiseConvolution2dLayer(desc, weights, optionalBiases, op.type().c_str());
                                            ...
                                            unsigned int kernel, unsigned int numGroups)
{
    ...
    auto it = blobs.find(op.input(1));
     ...
     unsigned int numFilters =static_cast<unsigned int>(w.arg(0).ints(0));
     ...
     for (unsigned int g = 0; g < numGroups; ++g)
    {
        ...
        if (convDesc.m_BiasEnabled)
        {
            ...
            convLayer =
                m_Network->AddConvolution2dLayer(convDesc, weights,optionalBiases, convLayerNames[g].c_str());
    ...
    unsigned int concatDimSizes[4] = {static_cast<unsigned int>(outputShape.ints(0)),
                                        static_cast<unsigned int>(outputShape.ints(1)),
    ...
    outputShape.set_ints(1, concatDimSizes[1]);
    // Finally add the merge layer
    IConnectableLayer* concatLayer = m_Network->AddConcatLayer(concatDesc, op.type().c_str());
  ...
  //std::cout<<op.output(0)<<" dfjgdfg\n";
     SetArmnnOutputSlotForCaffe2Output(op.output(0), concatLayer->GetOutputSlot(0));
}
 ...
 {    
    ...
    outputShape.add_ints(2);
    outputShape.set_ints(
        2, ((inputInfo.GetShape()[2] +  2 * pad - kernel)/
                            stride)+1);
    outputShape.add_ints(3);
    outputShape.set_ints(
        3, ((inputInfo.GetShape()[2] +  2 * pad - kernel)/
                            stride)+1);
    ...
    returnLayer = m_Network->AddConvolution2dLayer(convolution2dDescriptor, weights, optionalBiases, op.type().c_str());
    inputConnection.Connect(returnLayer->GetInputSlot(0));
    returnLayer->GetOutputSlot(0).SetTensorInfo(ArgumentToTensorInfo(outputShape));
